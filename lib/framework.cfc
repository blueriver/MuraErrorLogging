	...
	<cffunction name="doRequest">
		...
		<cfif isValidAction(rp)>
			...
			<cfif not listFindNoCase("none,layout", rp.vType)>
				<cfif fileExists('#expandPath("/#variables.pluginConfig.getPackage()#")#/#rp.section#/views/#rp.action#.cfm')>
					<cfsavecontent variable="$.rc.bodyContent"><cfinclude template="/#variables.pluginConfig.getPackage()#/#rp.section#/views/#rp.action#.cfm"></cfsavecontent>
				<cfelse>
					<cfthrow type="Action Error" message="The Action does not exist.">
				</cfif>
			</cfif>
			<cfif not listFindNoCase("none", rp.vType)>
				<cfif fileExists('#expandPath("/#variables.pluginConfig.getPackage()#")#/#rp.section#/layouts/#rp.layout#.cfm')>
					<cfinclude template="/#variables.pluginConfig.getPackage()#/#rp.section#/layouts/#rp.layout#.cfm">
